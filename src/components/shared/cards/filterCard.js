...
class FilterCard extends React.PureComponent {
  ...
  render() {
    let items = [];
    if (this.props.justCities) {
      items = [this.state.titles[0], this.state.titles[1]];
    } else if (this.props.limitedIndex) {
      items = [
        ...this.state.titles.slice(0, this.props.limitedIndex),
        ...this.state.titles.slice(this.props.limitedIndex + 1)
      ];
    } else {
